...
void CBasePlayer::UpdateClientSideAnimation() {
	bool bClientSideAnimation = m_bClientSideAnimation();
	hook_info.update_csa = m_bClientSideAnimation() = true;
	CallVFunction<void(__thiscall*)(CBasePlayer*)>(this, 224)(this);
	hook_info.update_csa = false;
	m_bClientSideAnimation() = bClientSideAnimation;
}
...
int CBasePlayer::GetAnimationLayerCount()
{
...
int CBasePlayer::GetSequenceActivity(int sequence)
{
	...
	auto hdr = ModelInfoClient->GetStudioModel(this->GetModel());

	if (!hdr)
		return 0;

	static auto sequence_activity = reinterpret_cast<int(__fastcall*)(void*, studiohdr_t*, int)>(Utils::PatternScan("client.dll", "55 8B EC 53 8B 5D 08 56 8B F1 83"));
...
float& CBasePlayer::m_flTimeOfLastInjury()
{
	static auto m_flTimeOfLastInjury = NetVars::GetNetVar("DT_BaseCombatCharacter", "m_flTimeOfLastInjury");
	return *(float*)(uintptr_t(this) + m_flTimeOfLastInjury);
}

float& CBasePlayer::m_flGroundAccelLinearFracLastTime()
{
	static auto m_flGroundAccelLinearFracLastTime = NetVars::GetNetVar("DT_CSPlayer", "m_flGroundAccelLinearFracLastTime");
