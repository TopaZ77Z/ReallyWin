...
public:
	...
	virtual	bool	ReadFromBuffer() = 0; // returns true if parsing was OK
	virtual	bool	WriteToBuffer() const = 0;	// returns true if writing was OK
...
typedef void(*pfnDemoCustomDataCallback)(int* pData, size_t iSize);
...
public:
	...
	virtual void SetCenter(Vector2& mappos) = 0; // set map pos in center of panel
	virtual void SetPlayerPositions(int index, const Vector& position, const QAngle& angle) = 0; // update player position
	virtual Vector2 WorldToMap(Vector& worldpos) = 0; // convert 3d world to 2d map pos
...
public:
	...
	virtual void                  DebugDrawPhysCollide(const CPhysCollide* pCollide, IMaterial* pMaterial, const float*& transform, const int* color) = 0; //79
