...
void GrenadePrediction::PrecacheParticles() {
	Effects->PrecacheParticleSystem("envfiretinyb");
...
void GrenadePrediction::Draw() {
	...
	if (weaponId == Molotov || weaponId == IncGrenade) {
		...
		CGameTrace trace;
		CTraceFilter filter;
		ray.Init(vecDetonate + Vector(0, 0, 1.25f), vecDetonate - Vector(0, 0, 120));
...
void GrenadeWarning::Warning(CBaseGrenade* entity, int weapId) {
	...
	if (weapId == Molotov) {
		...
		CTraceFilter filter;
		Ray_t ray;
		ray.Init(vecDetonate + Vector(0, 0, 0.01f), vecDetonate - Vector(0, 0, 120));
		filter.pSkip = owner;
	...
	for (int i = 1; i < pathPoints.size(); i++) {
		...
		GlowObjectManager->AddGlowBox(end, Math::VectorAngles(start - end), Vector(0, -0.5, -0.5), Vector((start - end).Q_Length(), 1, 1), config.visuals.other_esp.grenade_predict_color->get(), GlobalVars->frametime * 2.f);
	}
